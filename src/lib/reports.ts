...
export function dateRangeFromParams(searchParams: URLSearchParams): { dates: Date[]; dateStrs: string[]; from: string; to: string; isMonth: boolean; month: number; year: number } {
...
export async function computeDetail(searchParams: URLSearchParams) {
  const { dates, dateStrs } = dateRangeFromParams(searchParams);
  ...
  return { report: result, totalDays: dates.length };
}

export async function computeAttendance(searchParams: URLSearchParams) {
  const { dates, dateStrs } = dateRangeFromParams(searchParams);
  ...
  return { report, totalDays: dates.length };
}